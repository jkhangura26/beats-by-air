...
void continuous_dual_read() {
    float ax_bias1, ay_bias1, az_bias1;
    float ax_bias2, ay_bias2, az_bias2;

    quick_calibrate(MPU6050_ADDR1, &ax_bias1, &ay_bias1, &az_bias1);
    quick_calibrate(MPU6050_ADDR2, &ax_bias2, &ay_bias2, &az_bias2);
...
int main() {
    ...
    if (mpu6050_read_byte(MPU6050_ADDR1, WHO_AM_I, &id1) != I2C_SUCCESS || id1 != 0x68) {
        printf("Sensor 1 not detected. ID = 0x%02X\n", id1);
        close_i2c_bus();
        return 1;
    }

    if (mpu6050_read_byte(MPU6050_ADDR2, WHO_AM_I, &id2) != I2C_SUCCESS || id2 != 0x68) {
        printf("Sensor 2 not detected. ID = 0x%02X\n", id2);
        close_i2c_bus();
        return 1;
    }

    // Wake up both sensors and configure
    mpu6050_write_byte(MPU6050_ADDR1, PWR_MGMT_1, 0x00);
    mpu6050_write_byte(MPU6050_ADDR1, CONFIG, 0x02);

    mpu6050_write_byte(MPU6050_ADDR2, PWR_MGMT_1, 0x00);
    mpu6050_write_byte(MPU6050_ADDR2, CONFIG, 0x02);

    usleep(100000); // wait 100ms

    continuous_dual_read();
